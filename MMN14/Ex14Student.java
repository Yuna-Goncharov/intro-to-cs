public class Ex14Student {
    ...
    private static int numPaths(int[][] mat,   int x1, int y1, int x2, int y2, int count){
      char x;
      int temp;
      if (
          x1 < 0 || 
      ...
      if (x1 == x2 && y1 == y2) { 
        ...
        temp = mat[x1][y1]; //it saves the real value of the cell 
          
        mat[x1][y1]='x'; // exchange x with what's in the cell in order to know that we have been in this cell already
          
        count = numPaths(mat, x1+1, y1, x2, y2,count)
               + numPaths(mat, x1-1, y1, x2, y2,count)
               + numPaths(mat, x1, y1+1, x2, y2,count)
               + numPaths(mat, x1, y1-1, x2, y2,count);
